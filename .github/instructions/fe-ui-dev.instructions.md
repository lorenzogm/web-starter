...
- `.github/instructions/fe-contentful-blocks.instructions.md` - For Contentful integration with live updates and inspector mode
...
- Use `Text` component instead of `<h1>`, `<h2>`, `<p>`, etc. - supports variants: `h1`, `h2`, `h3`, `h4`, `body-md`
- Use `Image` component instead of `<img>` - supports `fill` mode with aspect ratios
...
### TextProps and ImageProps Pattern (for CMS Integration)

When building blocks that will receive data from a CMS (like Contentful), use the `TextProps` and `ImageProps` patterns to support live preview and inspector mode:

```typescript
type TextProps = {
  text: string;
  data?: Record<string, unknown>;  // For CMS inspector attributes
};
...
// In the component, spread the data props onto elements:
<Text variant="h2" {...props.title.data}>{props.title.text}</Text>
<Image src={props.image.src} alt={props.image.alt} {...props.image.data} />
```
